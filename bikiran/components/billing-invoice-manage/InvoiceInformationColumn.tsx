...
const InvoiceTitle: FC<{ data: TInvoiceData }> = ({ data }) => {
  const { openModal } = useTemplate();

  return (
    <div className="relative group bg-primary-200 rounded-10 mb-3 flex items-center print:hidden">
      <h2 className="z-0 text-primary w-full rounded-10 hover:bg-[#8080802e] text-lg font-medium text-left whitespace-nowrap text-ellipsis overflow-hidden px-4 py-6 group-hover:bg-[#8080802e]">
...
const InvoiceInformationColumn: FC = () => {
  const { invoiceInfo, loading, reload, scopes } = useInvoiceInfo();
