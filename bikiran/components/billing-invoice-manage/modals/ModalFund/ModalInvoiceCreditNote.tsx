...
import { ApiGetCreditNoteInfo } from "@/bikiran/components/billing/accountAdm/AccountAdmOperation";
import { ApiCreateInvoiceCreditNote } from "../../InvoiceManageOperation";
...
const ModalContent: FC = () => {
  ...
  useEffect(() => {
    ...
    ApiGetCreditNoteInfo(authInfo, modalData.invoice.billingId)
      .then(({ data }) => {
        setData(data);
      })
      .catch((err) => {
  ...
  const handleSubmit = () => {
    ...
    setLoading(true);
    ApiCreateInvoiceCreditNote(authInfo, modalData.invoice.id, payload)
      .then(({ message }) => {
        closeModal();
        reload();
        if (message) {
...
const ModalInvoiceCreditNote = () => {
  const { closeModal, modalType } = useTemplate();
  return (
    <Dialog open={modalType === "add-fund"} onOpenChange={closeModal}>
...
export default ModalInvoiceCreditNote;
