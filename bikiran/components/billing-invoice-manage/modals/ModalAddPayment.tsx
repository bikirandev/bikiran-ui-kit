...
import { ApiAddPayment } from "../InvoiceManageOperation";
...
const ModalContent: FC = () => {
  ...
  const [formData, setFormData] = useState<{
    ...
    payAmount: parseFloat(invoice.totalDue?.toFixed(2)) || 0,
    agree: false,
  });
  ...
  const newBalance = () => {
    if (
      (formData.payAmount > user.balance && formData.agree) ||
      formData.payAmount < user.balance
    ) {
  ...
  const handleAddPayment = async () => {
    ...
    setLoading(true);
    try {
      const { message } = await ApiAddPayment(
        authInfo,
        chkLoginReq,
        invoice?.id?.toString(),
        payload
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="w-full flex flex-col items-end ">
          <InputField
            ...
            className="max-w-[227px] !h-10 !text-sm"
