...
import CopyWrapper from "@/bik-lib/features/copy-wrapper/CopyWrapper";
...
const CustomerBalanceComp: FC<TProps> = ({ data, loading }) => {
  ...
  const { customerFac } = useInvoiceInfo();
    ...
    <div className="space-y-4">
      <div>
        <div className="flex items-center justify-between ">
          ...
          <InstOption disabled={loading}>
            <button
              onClick={() =>
                router.push(
                  `/billing/account/${customerFac?.id}/statement?dateFrom=${dayjs().subtract(1, "year").format("YYYY-MM-DD")}&dateTo=${dayjs().format("YYYY-MM-DD")}`
                )
              }
            >
              Statement
        ...
        <div className="flex items-center gap-1">
          <div className="text-primary-700 text-sm font-medium">FAC ID :</div>
          &nbsp;
          <CopyWrapper content={customerFac?.id} />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {customerFac?.currency}: {customerFac?.balance.toFixed(2)}
          </div>
