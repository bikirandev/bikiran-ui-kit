...
import { ApiSyncDown } from "../DomainListOperation";
import { useAuth2 } from "@/bik-lib/context/auth/Auth2Provider";
import { useRouter } from "next/navigation";
import { cn } from "@/bik-lib/utils/cn";
import TooltipWrapper from "@/bik-lib/lib/TooltipWrapper";
import TooltipUserInfo from "@/bikiran/shared/user-info/TooltipUserInfo";
import StatusColor from "@/bik-lib/utils/statusColor";
import { ApiDeleteDomain } from "../../domain-deleted/DomainDeletedListOperation";
...
const TableRow: FC<{ data: TDomainListItem }> = ({ data }) => {
  const { openModal, setMessage ,setConfirm ,setTemplateLoading} = useTemplate();
  ...
  const deleteDomain = (subsId: number) => {
      setConfirm({
        ...
        clickAction: () => {
          ...
          ApiDeleteDomain(authInfo, chkLoginReq,subsId)
            .then(({ message }) => {
              setMessage(message);
              //reload the chat
              reload();
  ...
  return (
    <tr className="overflow-visible">
      ...
      <td className="text-left items-center">
        <TooltipUserInfo user={data?.user} setShow={setShow} show={show} />
      </td>
        ...
        <div className="w-full flex justify-end">
          <InstOption className="size-7">
            ...
            <button
              ...
              onClick={(ev) => {
                ev.stopPropagation();
                router.push(`/renew?subscription-id=${data.id}&asset=domain`);
              }}
            >
              Renew
            </button>
            <button
              ...
              onClick={(ev) => {
                ev.stopPropagation();
                openModal("domain-title-update", data);
              }}
            >
              Update Title
            </button>
            ...
            <button
              ...
              onClick={(ev) => {
                ev.stopPropagation();
                openModal("domain-change-status", data);
              }}
            >
              Update Status
            </button>
            <button onClick={()=>deleteDomain(data.id)}>Remove Domain</button>
          </InstOption>
        </div>
      </td>
    </tr>
