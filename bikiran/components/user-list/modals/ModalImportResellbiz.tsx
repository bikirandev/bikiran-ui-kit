...
import {
  ApiAddImportFromResellbiz,
  ApiImportFromResellbiz,
} from "../UserListOperation";
...
const ModalContent: FC<TProps> = ({ closeModal, setMessage }) => {
  ...
  const handleImportUser = (ev: TInputChangeEvent) => {
    ...
    if (formData?.email?.length !== 0 && value !== payloadValue) {
      ...
      setLoading(true);
      ApiImportFromResellbiz(authInfo, chkLoginReq, formData)
        .then(({ data, message }) => {
          setMessage(message);
          setData(data);
  ...
  const handleAddUser = (ev: TFormEvent) => {
    ...
    setLoading(true);
    ApiAddImportFromResellbiz(authInfo, chkLoginReq, formData)
      .then(({ message }) => {
        setMessage(message);
        closeModal();
        reload();
