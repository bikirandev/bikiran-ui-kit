...
import { ApiUpdateUserPhoneStatus } from "../UserPhonesListOperation";
...
const ModalBody: FC<{
  ...
  const { reload, userPhonesData } = useUserPhonesList();
  ...
  const updateUserPhone = (payload: any) => {
    setLoading(true);
    setMessage("Updating...");
    ApiUpdateUserPhoneStatus(authInfo, chkLoginReq, modalData?.id || 0, payload)
      .then(({ message }) => {
        setMessage(message);
