...
import { ApiUserProfileView } from "../user-list/UserListOperation";
...
const UserHeader: FC<{ user: TOverview }> = ({ user }) => {
  return (
    <div className=" w-full flex justify-between items-center py-4 border-b border-gray-200 mb-4 top-0 bg-white z-10">
...
const UserOverviewPage: FC<{
  ...
  useEffect(() => {
    if (userId !== 0) {
      setLoading(true);
      ApiUserProfileView(authInfo, chkLoginReq, Number(id))
        .then(({ data }) => {
          setUserData(data);
        })
        .catch((err: Error) => {
