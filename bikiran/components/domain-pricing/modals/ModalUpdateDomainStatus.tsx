...
import { ApiUpdateDomainStatus } from "../DomainOperation";
...
const ModalBody: FC<{
  ...
  const updateStatus = (payload: any) => {
    ...
    ApiUpdateDomainStatus(authInfo, chkLoginReq, modalData.id, payload)
      .then(({ message }) => {
        setMessage(message);
        closeModal();
        reFetch();
...
const ModalUpdateDomainStatus: FC<{}> = () => {
  const { closeModal, modalType, modalData } = useTemplate();

  return (
    <Dialog
      open={modalType === "update-domain-status"}
...
export default ModalUpdateDomainStatus;
