...
import { ApiLoadInvoiceInfo } from "../billing-invoice-manage/InvoiceManageOperation";
...
const InvoiceSidebarSection: FC<{ data: any }> = ({ data }) => {
  const [invoiceInfo, setInvoiceInfo] = React.useState<TInvoiceData>(
    {} as TInvoiceData
  );
  ...
  useEffect(() => {
    if (data > 0) {
      setLoading(true);
      ApiLoadInvoiceInfo(authInfo, chkLoginReq, data as string)
        .then(({ data }) => {
          if (data) {
            setInvoiceInfo(data);
          }
      ...
      {data ? (
        <div>
          ...
          <div className="[&>div]:!p-0 [&>div]:shadow-none [&>div]:!bg-transparent">
            ...
            {invoiceInfo?.invoice?.id === 0 && !loading && (
          ...
          <button
            ...
            onClick={() => router.push(`/billing/invoice/${data}/update`)}
          >
            <svg
              width="50"
              height="50"
